...
# ── Global log queue ─────────────────────────────────────────
# Background thread → queue → main thread reads on rerun (no ScriptRunContext needed)
...
with st.sidebar:
    ...
    concurrency = st.slider("Workers (parallel)", 1, 20, 10, help="Paid key: 10–15. Free key: 3.")
...
if run_clicked and competitor_domain and not st.session_state.running:
    ...
    rw_mod.CONCURRENCY = int(concurrency)   # apply slider value
    ...
    def run_thread():
        try:
            enqueue(f"▶ Starting  |  domain:{competitor_domain}  stages:{stages}  workers:{concurrency}  dry_run:{dry_run}")
